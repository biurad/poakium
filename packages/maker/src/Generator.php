...
class Generator
{
     ...
     * @param string|null $templateName Template name in Resources/skeleton to use
    ...
    public function generateClass($classNameDetails, ?string $templateName, array $variables = []): string
    {
        ...
        if (null === $targetPath) {
            throw new \LogicException(sprintf('Could not determine where to locate the new class "%s", maybe try with a full namespace like "\\My\\Full\\Namespace\\%s"', $className, HelperUtil::getShortClassName($className)));
        }
    ...
    private function addOperation(string $targetPath, $templateName, array $variables)
    {
