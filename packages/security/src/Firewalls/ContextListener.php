...
use BiuradPHP\Security\Event\DeauthenticatedEvent;
...
class ContextListener extends AbstractListener
{
     ...
     * @param callable|null $sessionTrackerEnabler
     */
    ...
    public function authenticate(RequestEvent $event)
    {
        if (!$this->registered && null !== $this->dispatcher) {
            $this->dispatcher->addListener(SecurityEvents::INTERACTIVE_LOGIN, [$this, 'onKernelResponse'], 0);
        ...
        if (null !== $session) {
            ...
            $token = $session->getSection()->get($this->sessionKey);
     ...
     */
    public function onKernelResponse(InteractiveLoginEvent $event)
    {
        $request = $event->getRequest();
        ...
        if (null === $token || $this->trustResolver->isAnonymous($token)) {
            if ($request->hasCookie($session->getName())) {
                $session->getSection()->delete($this->sessionKey);
            }
        } else {
            $session->getSection()->set($this->sessionKey, serialize($token));
    ...
    protected function refreshUser(TokenInterface $token): ?TokenInterface
    {
        ...
        if ($userDeauthenticated) {
            ...
            if (null !== $this->dispatcher) {
                $this->dispatcher->dispatch(new DeauthenticatedEvent($token, $newToken));
            }
